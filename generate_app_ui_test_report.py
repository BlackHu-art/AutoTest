...
if __name__ == '__main__':
    ...
    if 'Windows' == platform.system():
        ...
        for i in range(len(report_dirs)):
            ...
            try:
                ...
                try:
                    ...
                    DateTimeTool.getNowTime(), get_allure_process_id, port))
            except:
        ...
        for i in range(len(report_dirs)):
            ...
            for port_process_id in port_process_ids:
                ...
                for allure_process_id in allure_process_ids:
                    ...
                    if allure_process_id == port_process_id and not is_find and allure_process_id and port_process_id:
                        print('%s关闭allure进程,进程id:%s,该进程监听已监听端口:%s' % (
                        DateTimeTool.getNowTime(), allure_process_id.strip(), port))
            ...
            print('%s报告地址:http://%s:%s/' % (DateTimeTool.getNowTime(), Network.get_local_ip(), port))
            generate_report_command = 'allure generate %s/report_data -o %s/report/app_ui_report_%s' % (
            report_dirs[i], test_time)
            subprocess.check_output(generate_report_command, shell=True)
            open_report_command = 'nohup allure open -p %s %s/report/app_ui_report_%s >logs/generate_app_ui_test_report_%s.log 2>&1 &' % (
            port, report_dirs[i], test_time, test_time)
            subprocess.check_output(open_report_command, shell=True)
